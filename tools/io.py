...
def readTimeCurrentData(file_name):
...
def writeTimeCurrentDataBinaryCompressed(fileName, t, I, overwrite=False):
    ...
    if len(t) != len(I):
        msg = """Expected time and current lists to have the same length.
                 Time length was {0} and current length was
                 {1}.""".format(len(t), len(I))
        raise ValueError(msg)
...
def writeTimeCurrentDataBinaryUncompressed(fileName, t, I, overwrite=False):
    ...
    if len(t) != len(I):
        msg = "Expected time and current lists to have the same length." +\
                 "Time length was {0} and current length " + \
                 "was {1}.".format(len(t), len(I))
        raise ValueError(msg)
    ...
    for entry in loaded:
        if entry['name'] == dataName:
            if not typeName is None and entry['type'] != typeName:
                msg_unformatted = "Parameters {0} in file {1} are not"\
                                + " nondimensional.  The parameters have"\
                                + " type {2}"
                mesg = msg.format(dataName, fileName, entry['type'])
                raise ValueError(msg)
            return entry
    return None
...
def writeNondimensionalParametersToJSON(
          fileName, dataName, eps_0, dEps, omega, kappa, rho, gamma, gamma1,
          gamma2, gamma3, revTau):
    ...
    will be overwritten.
    """
    parameters = {'name' : dataName,'type' : 'nondimensional', 'eps_0': eps_0,
                  'dEps' : dEps, 'omega' : omega, 'kappa' : kappa,
                  'rho' : rho, 'gamma' : gamma, 'gamma1' : gamma1,
                  'gamma2' : gamma2, 'gamma3' : gamma3, 'revTau' : revTau}
    writeParametersToJSON(fileName, parameters)
...
Ru, Cdl, Cdl1, Cdl2, Cdl3, EStart, ERev, temp, nu, area, coverage, reverse):
    ...
    will be overwritten.
    """
    parameters = {'name' : dataName,'type' : 'dimensional', 'eps_0': eps_0,
                  'dEps' : dEps, 'omega' : omega, 'kappa' : kappa, 'rho' : rho,
                  'Cdl' : Cdl, 'Cdl1' : Cdl1,'Cdl2' : Cdl2, 'Cdl3' : Cdl3,
