...
class ACExperiment(object):
    ...
    def _capacitanceAndFreqObjectiveFunction(self, capFreqParams):
        ...
        try:
            I, _ = st.solve_reaction_dimensional(
                    self.time_step, self.num_time_pts, self.E_0, self.dE, freq,
                    self.k_0, self.Ru, Cdl, Cdl1, Cdl2, Cdl3, self.EStart,
                    self.ERev, self.temp, self.nu, self.area, self.coverage)
    ...
    def _timeDomainObjectiveFunction(self, params):
        ...
        try:
            I, _ = st.solve_reaction_dimensional(
                    self.time_step, self.num_time_pts, E_0, self.dE, freq,
                    k_0, self.Ru, Cdl, Cdl1, Cdl2, Cdl3, self.EStart,
                    self.ERev, self.temp, self.nu, self.area, self.coverage)
    ...
    def plotFitDiscrepancy(self):
        ...
        """
        IFit, _ = st.solve_reaction_dimensional(
                self.time_step, self.num_time_pts, self.E_0, self.dE,
                        self.freq, self.k_0, self.Ru, self.Cdl, self.Cdl1,
                        self.Cdl2, self.Cdl3, self.EStart, self.ERev,
                        self.temp, self.nu, self.area, self.coverage)
