...
class SolutionToolsTests(unittest.TestCase):
    """Tests for tools.solution_tools"""
    ...
    def testMatchesMartinsData(self):
        ...
        time, IObs =  io.read_time_current_data_bin(fileName)
        time_step = time[1] - time[0]
        num_time_pts = len(time)

        params = io.read_json_dimen_params(jsonFileName, dataName)

        I, _ = st.solve_reaction_from_json(time_step, num_time_pts,
                                                       params)

        lower = int(np.ceil(0.2 * num_time_pts))
        upper = int(np.floor(0.8 * num_time_pts))
        ind = range(lower, upper)

        #Check that model accounts for >= 95% of variation about the mean.
        self.assertTrue(np.sum(np.square(IObs[ind] - I[ind]))
                        <= 25e-4 * np.sum(np.square(IObs[ind] - np.mean(IObs[ind]))))
